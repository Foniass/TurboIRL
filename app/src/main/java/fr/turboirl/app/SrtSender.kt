 ...
 *
 * Deliberate (24/09): the whole backlog lives in libsrt's send buffer, so when the uplink saturates for
 * longer than the latency, libsrt discards the oldest packets, audio included (holes of 0.1-0.9 s in the
 * sound), and the stream keeps its fixed delay. The alternative, keeping the backlog in this class and
 * sending the audio late but whole (2.2), was tried and rejected: the delay grew with every saturation.
 */
class SrtSender(
